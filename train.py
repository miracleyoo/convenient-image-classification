    ...
    for i, data in tqdm(enumerate(test_loader), desc="Testing", total=len(test_loader), leave=False, unit='b'):
        ...
        for i, predict in enumerate(predicts):
            if predict in list(np.where(labels_data[i]==1)[0]):
                num_correct += 1
            results.append((class_names[predict], data_path))
...
def predicting(opt, test_loader, net, class_names):
    net.eval()
    # top_num = opt.TOP_NUM
    results  = []
    criterion = nn.BCEWithLogitsLoss(size_average=False)
    ...
    for i, data in tqdm(enumerate(test_loader), desc="Testing", total=len(test_loader), leave=False, unit='b'):
        ...
        for predict, dp in zip(predicts,data_path):
            results.append((class_names[predict], dp))
